...
fn deterministic_seed() -> Result[Unit, Str] {
  # Two runs with the same seed should produce the same outcome.
  let a := ck.check_random(sometimes_false_spec(), 50, 123)
  let b := ck.check_random(sometimes_false_spec(), 50, 123)
  if ck.check_result_label(a) == ck.check_result_label(b) { Ok(()) } else {
...
fn suite() -> List[Result[Unit, Str]] {
  [
    holds_finds_no_counter(),
    falsifies_negative_x(),
    render_counter_includes_binding(),
    deterministic_seed(),
  ]
}
