...
fn binding_x(n :: Int) -> List[(Str, sp.SpecValue)] { [("x", VInt(n))] }
...
fn eval_allow_simple() -> Result[Unit, Str] {
  ...
  if sp.verdict_is_allow(v) { Ok(()) } else { Err(sp.verdict_label(v)) }
}
...
fn eval_and_or() -> Result[Unit, Str] {
  let spec := {
    ...
    quantifiers: [QInt("x")],
    predicate: EAnd(
      EBinop({ op: ">", lhs: EVar("x"), rhs: EConst(VInt(0)) }),
      EOr(
        EBinop({ op: "<", lhs: EVar("x"), rhs: EConst(VInt(100)) }),
        EConst(VBool(false))
      )
    ),
  }
...
fn eval_implies() -> Result[Unit, Str] {
  let spec := {
    ...
    predicate: EImplies(
      EBinop({ op: ">", lhs: EVar("x"), rhs: EConst(VInt(0)) }),
      EBinop({ op: ">=", lhs: EVar("x"), rhs: EConst(VInt(1)) })
    ),
  }
  let r := ev.eval(spec, [("x", VInt(5))])
  if sp.verdict_is_allow(r) { Ok(()) } else { Err("implies should hold") }
}
