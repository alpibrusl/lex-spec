  ...
  match sp.lookup_binding(bindings, binding) {
    ...
    Some(v) => match v {
      VRecord(rec) => match sp.record_field(rec, field) {
        None    => EvalInc(str.concat("missing field: ",
          str.concat(binding, str.concat(".", field)))),
...
fn values_equal(a :: sp.SpecValue, b :: sp.SpecValue) -> Bool {
  match a {
    VInt(n)   => match b { VInt(m)   => n == m,         _ => false },
    VStr(s)   => match b { VStr(t)   => s == t,         _ => false },
    VBool(x)  => match b { VBool(y)  => x == y,         _ => false },
    VFloat(x) => match b { VFloat(y) => x == y,         _ => false },
    VNull     => match b { VNull     => true,           _ => false },
    VRecord(r1) => match b { VRecord(r2) => r1.name == r2.name, _ => false },
    VList(_)  => false,
  }
}
...
fn apply_arith(op :: Str, a :: Int, b :: Int) -> EvalResult {
  if op == "+" { EvalVal(VInt(a + b)) }
  else { if op == "-" { EvalVal(VInt(a - b)) }
  else { if op == "*" { EvalVal(VInt(a * b)) }
  else { if op == "%" {
    if b == 0 { EvalInc("modulo by zero") }
    else { EvalVal(VInt(a - (a / b) * b)) }
  }
  else { EvalInc(str.concat("unsupported arith op: ", op)) }}}}
}
