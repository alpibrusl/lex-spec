...
type CapDirection = Inbound | Outbound | Local | EscalateHuman
...
type Capability = { name :: Str, description :: Str, params :: sch.ModelSchema, reply :: Option[sch.ModelSchema], direction :: CapDirection, precondition :: Option[sp.Spec] }

# Convenience constructors --------------------------------------------
fn inbound(name :: Str, description :: Str, params :: sch.ModelSchema) -> Capability {
  { name: name, description: description, params: params, reply: None, direction: Inbound, precondition: None }
}

fn outbound(name :: Str, description :: Str, params :: sch.ModelSchema) -> Capability {
  { name: name, description: description, params: params, reply: None, direction: Outbound, precondition: None }
}

fn with_reply(cap :: Capability, reply :: sch.ModelSchema) -> Capability {
  { name: cap.name, description: cap.description, params: cap.params, reply: Some(reply), direction: cap.direction, precondition: cap.precondition }
}

fn with_precondition(cap :: Capability, pre :: sp.Spec) -> Capability {
  { name: cap.name, description: cap.description, params: cap.params, reply: cap.reply, direction: cap.direction, precondition: Some(pre) }
}
...
fn gate(cap :: Capability, bindings :: List[(Str, sp.SpecValue)]) -> sp.Verdict {
...
fn allows(cap :: Capability, bindings :: List[(Str, sp.SpecValue)]) -> Bool {
