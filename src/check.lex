...
type CheckResult =
    Holds(Int)                       # n cases checked, all pass
  | Falsified(Counter)
  | Indeterminate(Counter)           # found Inconclusive — likely a typo
...
fn generate_row(qs :: List[sp.SpecQuant], rng :: Int) -> Row {
  list.fold(qs, { bindings: [], rng: rng },
    fn (acc :: Row, q :: sp.SpecQuant) -> Row {
      let stepped := mix(acc.rng)
      let pair := generate_for(q, stepped)
      {
        bindings: list.concat(acc.bindings, [(sp.quant_binding_name(q), pair.value)]),
        rng:      pair.rng,
      }
    })
}
...
fn generate_for(q :: sp.SpecQuant, rng :: Int) -> Gen {
  match q {
    ...
    QList(_)  => { value: VList([]), rng: mix(rng) },   # bounded — empty list
  }
}

fn generate_record(
  r :: { name :: Str, fields :: List[{ name :: Str, ty :: sp.Type }] },
  rng :: Int
) -> Gen {
  let walked := list.fold(r.fields, { entries: [], rng: rng },
    fn (
      acc :: { entries :: List[(Str, sp.SpecValue)], rng :: Int },
      f :: { name :: Str, ty :: sp.Type }
    ) -> { entries :: List[(Str, sp.SpecValue)], rng :: Int } {
      let stepped := mix(acc.rng)
      let g := generate_for_type(f.ty, stepped)
      {
        entries: list.concat(acc.entries, [(f.name, g.value)]),
        rng: g.rng,
      }
    })
...
fn generate_for_type(t :: sp.Type, rng :: Int) -> Gen {
  match t {
    ...
    TRecord(_) => { value: VNull, rng: mix(rng) },  # nested records deferred
...
fn int_in_range(s :: Int, lo :: Int, hi :: Int) -> Int {
  let span := hi - lo + 1
  if span <= 0 { lo } else { lo + int_mod(s, span) }
}
...
fn pick_str(s :: Int) -> Str {
  ...
  # `std.list` doesn't ship a `drop`; we fold with an index counter.
  let walked := list.fold(list.enumerate(pool), "",
...
fn render_counter(c :: Counter) -> Str {
  let lines := list.map(c.bindings,
    fn (pair :: (Str, sp.SpecValue)) -> Str {
      match pair {
        (k, v) => str.concat("  ", str.concat(k,
          str.concat(" = ", sp.show_value(v))))
      }
    })
...
fn render_check_result(r :: CheckResult) -> Str {
  match r {
    Holds(n) => str.concat("ok: predicate held over ",
      str.concat(int.to_str(n), " random cases")),
