...
type SpecValue =
  ...
  | VFloat(Float)
  | VRecord(RecordValue)
  | VList(List[SpecValue])
  | VNull

# A RecordValue is the bag of (field-name → SpecValue) used by
# `EField` lookups. Kept as a list-of-pairs to stay deterministic
# across operating systems / hash-map orderings (canonicalization
# rule 1 — same value → same form).
type RecordValue = {
  name :: Str,
  fields :: List[(Str, SpecValue)],
}
...
type SpecExpr =
    EField({ binding :: Str, field :: Str })
  | EVar(Str)                       # plain (non-record) binding ref
  | EConst(SpecValue)
...
# Supported binop ops — the canonical strings the evaluator
...
fn record_field(rec :: RecordValue, field :: Str) -> Option[SpecValue] {
  list.fold(rec.fields, None, fn (acc :: Option[SpecValue], pair :: (Str, SpecValue)) -> Option[SpecValue] {
    match acc {
      Some(_) => acc,
      None    => match pair { (k, v) => if k == field { Some(v) } else { None } },
    }
  })
}
...
fn show_expr(e :: SpecExpr) -> Str {
  match e {
    ...
    EBinop(b)   => str.concat("(",
      str.concat(show_expr(b.lhs),
        str.concat(" ",
          str.concat(b.op,
            str.concat(" ",
              str.concat(show_expr(b.rhs), ")")))))),
    ENot(x)     => str.concat("(not ", str.concat(show_expr(x), ")")),
    EAnd(a, b)  => str.concat("(", str.concat(show_expr(a),
      str.concat(" and ", str.concat(show_expr(b), ")")))),
    EOr(a, b)   => str.concat("(", str.concat(show_expr(a),
      str.concat(" or ",  str.concat(show_expr(b), ")")))),
    EImplies(a, b) => str.concat("(", str.concat(show_expr(a),
      str.concat(" => ",  str.concat(show_expr(b), ")")))),
  }
}
